import React from 'react';

const ChakraReleasing = () => {
  return (
    <div
      className=" overflow-hidden bg-white py-24 md:py-28 lg:py-30 px-6 sm:px-10 xl:px-20"
      id="releasing"
    >
      <div className="max-w-[1280px] mx-auto overflow-hidden flex justify-between gap-9 sm:gap-10 lg:gap-16 items-center flex-col sm:flex-row">
        <div className="sm:min-h-64 flex flex-col justify-between items-start">
          <div className="text-[#451e17] text-2xl sm:text-[32px] lg:text-[40px] font-medium capitalize leading-[28.8px] sm:leading-[38.4px] lg:leading-[48px] lg:w-[700px]">
            <span className="italic text-[#b67238] text-[21px] sm:text-[29px] lg:text-[37px]">
              Releasing $1T Bitcoin liquidity
            </span>{' '}
            and interoperability for each chain and application.
          </div>

          <div className="hidden sm:inline-flex justify-end items-center gap-8">
            <div className="text-right text-[#451e17] text-xl font-normal uppercase leading-normal sm:text-base sm:leading-tight">
              media
            </div>
            <div className="flex justify-end gap-6 items-center sm:justify-center">
              <a
                href="https://t.me/ChakraProtocol"
                target="_blank"
                className="cursor-pointer"
              >
                <div>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
                    fill="none"
                  >
                    <path
                      fillRule="evenodd"
                      clipRule="evenodd"
                      d="M9.52401 19.2672L9.83464 14.5733L18.3568 6.89441C18.7341 6.55053 18.2792 6.38405 17.7798 6.68362L7.26031 13.3306L2.71073 11.8879C1.73418 11.6105 1.72313 10.9336 2.93261 10.4453L20.6538 3.60995C21.4639 3.24369 22.2407 3.80959 21.93 5.05243L18.9117 19.2672C18.7009 20.2769 18.0905 20.5211 17.2472 20.0551L12.6532 16.6595L10.445 18.8012C10.1898 19.0563 9.97892 19.2672 9.52401 19.2672Z"
                      fill="#121212"
                    />
                  </svg>
                </div>
              </a>
              <a
                href="https://twitter.com/ChakraChain"
                target="_blank"
                className="cursor-pointer"
              >
                <div>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
                    fill="none"
                  >
                    <path
                      d="M17.4636 3.5H20.3771L14.012 10.7748L21.5 20.6742H15.637L11.0448 14.6703L5.79041 20.6742H2.87519L9.68324 12.893L2.5 3.5H8.51187L12.6628 8.98783L17.4636 3.5ZM16.4411 18.9304H18.0555L7.63466 5.15225H5.90226L16.4411 18.9304Z"
                      fill="#121212"
                    />
                  </svg>
                </div>
              </a>
              <a
                href="https://discord.gg/chakrachain"
                target="_blank"
                className="cursor-pointer"
              >
                <div>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
                    fill="none"
                  >
                    <path
                      d="M18.9308 5.76368C17.6561 5.17878 16.2892 4.74785 14.8599 4.50104C14.8339 4.49627 14.8079 4.50818 14.7945 4.53198C14.6187 4.84466 14.4239 5.25258 14.2876 5.5732C12.7503 5.34306 11.221 5.34306 9.71527 5.5732C9.57887 5.24545 9.37707 4.84466 9.20048 4.53198C9.18707 4.50897 9.16107 4.49707 9.13504 4.50104C7.70659 4.74706 6.33963 5.17799 5.06411 5.76368C5.05307 5.76844 5.04361 5.77638 5.03732 5.78669C2.44449 9.66033 1.73421 13.4387 2.08265 17.1703C2.08423 17.1886 2.09447 17.206 2.10867 17.2171C3.81934 18.4734 5.47642 19.2361 7.10273 19.7416C7.12876 19.7496 7.15634 19.74 7.1729 19.7186C7.55761 19.1933 7.90054 18.6393 8.19456 18.0568C8.21192 18.0227 8.19535 17.9822 8.15989 17.9687C7.61594 17.7624 7.098 17.5108 6.59977 17.2251C6.56037 17.2021 6.55721 17.1457 6.59347 17.1187C6.69831 17.0402 6.80318 16.9584 6.9033 16.8759C6.92141 16.8608 6.94665 16.8576 6.96794 16.8671C10.2411 18.3615 13.7846 18.3615 17.0191 16.8671C17.0404 16.8568 17.0657 16.86 17.0846 16.8751C17.1847 16.9576 17.2895 17.0402 17.3952 17.1187C17.4314 17.1457 17.4291 17.2021 17.3897 17.2251C16.8914 17.5163 16.3735 17.7624 15.8288 17.9679C15.7933 17.9814 15.7775 18.0227 15.7949 18.0568C16.0952 18.6385 16.4381 19.1924 16.8157 19.7178C16.8315 19.74 16.8599 19.7496 16.8859 19.7416C18.5201 19.2361 20.1772 18.4734 21.8879 17.2171C21.9028 17.206 21.9123 17.1894 21.9139 17.1711C22.3309 12.857 21.2154 9.10956 18.9568 5.78748C18.9513 5.77638 18.9419 5.76844 18.9308 5.76368ZM8.68335 14.8982C7.69792 14.8982 6.88594 13.9935 6.88594 12.8824C6.88594 11.7713 7.68217 10.8666 8.68335 10.8666C9.69239 10.8666 10.4965 11.7793 10.4807 12.8824C10.4807 13.9935 9.68451 14.8982 8.68335 14.8982ZM15.329 14.8982C14.3435 14.8982 13.5316 13.9935 13.5316 12.8824C13.5316 11.7713 14.3278 10.8666 15.329 10.8666C16.338 10.8666 17.1421 11.7793 17.1264 12.8824C17.1264 13.9935 16.338 14.8982 15.329 14.8982Z"
                      fill="#121212"
                    />
                  </svg>
                </div>
              </a>
              <a
                href="https://medium.com/@chakrachainbtc"
                target="_blank"
                className="cursor-pointer"
              >
                <div>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
                    fill="none"
                  >
                    <path
                      d="M13.3495 11.8125C13.3495 15.0203 10.7031 17.625 7.42476 17.625C4.14639 17.625 1.5 15.0203 1.5 11.8125C1.5 8.60472 4.14639 6 7.42476 6C10.7031 6 13.3495 8.60472 13.3495 11.8125ZM19.8404 11.8125C19.8404 14.8278 18.5107 17.2786 16.8781 17.2786C15.2455 17.2786 13.9157 14.8278 13.9157 11.8125C13.9157 8.79718 15.2455 6.34644 16.8781 6.34644C18.5107 6.34644 19.8404 8.78435 19.8404 11.8125ZM22.5 11.8125C22.5 14.5199 22.0392 16.714 21.4599 16.714C20.8806 16.714 20.4197 14.5199 20.4197 11.8125C20.4197 9.10513 20.8806 6.91101 21.4599 6.91101C22.0392 6.91101 22.5 9.10513 22.5 11.8125Z"
                      fill="#121212"
                    />
                  </svg>
                </div>
              </a>
              <a
                href="https://mirror.xyz/0xa3564d6065379D3cd262B7a3a88B0CBcf686B1f8"
                target="_blank"
                className="cursor-pointer"
              >
                <div>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
                    fill="none"
                  >
                    <path
                      d="M5.5 9.69355C5.5 6.27295 8.41015 3.5 12 3.5C15.5899 3.5 18.5 6.27295 18.5 9.69355V18.5589C18.5 19.0786 18.0578 19.5 17.5122 19.5H6.48775C5.94223 19.5 5.5 19.0786 5.5 18.5589V9.69355Z"
                      fill="#121212"
                    />
                  </svg>
                </div>
              </a>
            </div>
          </div>
        </div>

        <div>
          {/* Desktop image */}
          <img
            alt="Chakra Protocol"
            className="hidden lg:block"
            width="256"
            height="256"
            src="/slides/content1.svg"
          />

          {/* Mobile image */}
          <img
            alt="Chakra Protocol"
            className="block lg:hidden"
            width="344"
            height="256"
            src="/slides/content1.svg"
          />
        </div>

        {/* Mobile social links */}
        <div className="sm:hidden inline-flex justify-start items-center gap-8">
          <div className="text-right text-[#451e17] text-xl font-normal uppercase leading-normal">
            media
          </div>
          <div className="flex justify-end gap-6 items-center justify-center">
            <a
              href="https://t.me/ChakraProtocol"
              target="_blank"
              className="cursor-pointer"
            >
              <div>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                >
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M9.52401 19.2672L9.83464 14.5733L18.3568 6.89441C18.7341 6.55053 18.2792 6.38405 17.7798 6.68362L7.26031 13.3306L2.71073 11.8879C1.73418 11.6105 1.72313 10.9336 2.93261 10.4453L20.6538 3.60995C21.4639 3.24369 22.2407 3.80959 21.93 5.05243L18.9117 19.2672C18.7009 20.2769 18.0905 20.5211 17.2472 20.0551L12.6532 16.6595L10.445 18.8012C10.1898 19.0563 9.97892 19.2672 9.52401 19.2672Z"
                    fill="#121212"
                  />
                </svg>
              </div>
            </a>
            <a
              href="https://twitter.com/ChakraChain"
              target="_blank"
              className="cursor-pointer"
            >
              <div>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                >
                  <path
                    d="M17.4636 3.5H20.3771L14.012 10.7748L21.5 20.6742H15.637L11.0448 14.6703L5.79041 20.6742H2.87519L9.68324 12.893L2.5 3.5H8.51187L12.6628 8.98783L17.4636 3.5ZM16.4411 18.9304H18.0555L7.63466 5.15225H5.90226L16.4411 18.9304Z"
                    fill="#121212"
                  />
                </svg>
              </div>
            </a>
            <a
              href="https://discord.gg/chakrachain"
              target="_blank"
              className="cursor-pointer"
            >
              <div>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                >
                  <path
                    d="M18.9308 5.76368C17.6561 5.17878 16.2892 4.74785 14.8599 4.50104C14.8339 4.49627 14.8079 4.50818 14.7945 4.53198C14.6187 4.84466 14.4239 5.25258 14.2876 5.5732C12.7503 5.34306 11.221 5.34306 9.71527 5.5732C9.57887 5.24545 9.37707 4.84466 9.20048 4.53198C9.18707 4.50897 9.16107 4.49707 9.13504 4.50104C7.70659 4.74706 6.33963 5.17799 5.06411 5.76368C5.05307 5.76844 5.04361 5.77638 5.03732 5.78669C2.44449 9.66033 1.73421 13.4387 2.08265 17.1703C2.08423 17.1886 2.09447 17.206 2.10867 17.2171C3.81934 18.4734 5.47642 19.2361 7.10273 19.7416C7.12876 19.7496 7.15634 19.74 7.1729 19.7186C7.55761 19.1933 7.90054 18.6393 8.19456 18.0568C8.21192 18.0227 8.19535 17.9822 8.15989 17.9687C7.61594 17.7624 7.098 17.5108 6.59977 17.2251C6.56037 17.2021 6.55721 17.1457 6.59347 17.1187C6.69831 17.0402 6.80318 16.9584 6.9033 16.8759C6.92141 16.8608 6.94665 16.8576 6.96794 16.8671C10.2411 18.3615 13.7846 18.3615 17.0191 16.8671C17.0404 16.8568 17.0657 16.86 17.0846 16.8751C17.1847 16.9576 17.2895 17.0402 17.3952 17.1187C17.4314 17.1457 17.4291 17.2021 17.3897 17.2251C16.8914 17.5163 16.3735 17.7624 15.8288 17.9679C15.7933 17.9814 15.7775 18.0227 15.7949 18.0568C16.0952 18.6385 16.4381 19.1924 16.8157 19.7178C16.8315 19.74 16.8599 19.7496 16.8859 19.7416C18.5201 19.2361 20.1772 18.4734 21.8879 17.2171C21.9028 17.206 21.9123 17.1894 21.9139 17.1711C22.3309 12.857 21.2154 9.10956 18.9568 5.78748C18.9513 5.77638 18.9419 5.76844 18.9308 5.76368ZM8.68335 14.8982C7.69792 14.8982 6.88594 13.9935 6.88594 12.8824C6.88594 11.7713 7.68217 10.8666 8.68335 10.8666C9.69239 10.8666 10.4965 11.7793 10.4807 12.8824C10.4807 13.9935 9.68451 14.8982 8.68335 14.8982ZM15.329 14.8982C14.3435 14.8982 13.5316 13.9935 13.5316 12.8824C13.5316 11.7713 14.3278 10.8666 15.329 10.8666C16.338 10.8666 17.1421 11.7793 17.1264 12.8824C17.1264 13.9935 16.338 14.8982 15.329 14.8982Z"
                    fill="#121212"
                  />
                </svg>
              </div>
            </a>
            <a
              href="https://medium.com/@chakrachainbtc"
              target="_blank"
              className="cursor-pointer"
            >
              <div>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                >
                  <path
                    d="M13.3495 11.8125C13.3495 15.0203 10.7031 17.625 7.42476 17.625C4.14639 17.625 1.5 15.0203 1.5 11.8125C1.5 8.60472 4.14639 6 7.42476 6C10.7031 6 13.3495 8.60472 13.3495 11.8125ZM19.8404 11.8125C19.8404 14.8278 18.5107 17.2786 16.8781 17.2786C15.2455 17.2786 13.9157 14.8278 13.9157 11.8125C13.9157 8.79718 15.2455 6.34644 16.8781 6.34644C18.5107 6.34644 19.8404 8.78435 19.8404 11.8125ZM22.5 11.8125C22.5 14.5199 22.0392 16.714 21.4599 16.714C20.8806 16.714 20.4197 14.5199 20.4197 11.8125C20.4197 9.10513 20.8806 6.91101 21.4599 6.91101C22.0392 6.91101 22.5 9.10513 22.5 11.8125Z"
                    fill="#121212"
                  />
                </svg>
              </div>
            </a>
            <a
              href="https://mirror.xyz/0xa3564d6065379D3cd262B7a3a88B0CBcf686B1f8"
              target="_blank"
              className="cursor-pointer"
            >
              <div>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                >
                  <path
                    d="M5.5 9.69355C5.5 6.27295 8.41015 3.5 12 3.5C15.5899 3.5 18.5 6.27295 18.5 9.69355V18.5589C18.5 19.0786 18.0578 19.5 17.5122 19.5H6.48775C5.94223 19.5 5.5 19.0786 5.5 18.5589V9.69355Z"
                    fill="#121212"
                  />
                </svg>
              </div>
            </a>
          </div>
        </div>
      </div>
    </div>
  );
};

export default ChakraReleasing;
